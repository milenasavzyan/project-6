...
<div class="head text-center">
    <div class="element row col-md-4 thumbnail mx-auto" >
        <?php if ($product):?>
                <div class="product">
                    <h2><?= $product['name']; ?></h2>
                    <img class="rounded mx-auto d-block" src="<?= $product['image_path']; ?>" alt="<?= $product['name']; ?>" style="width:70%">
    ...
    <form method="post" >
        ...
        <a href="../public/index.php?route=cart&product_id=<?= $product['product_id']; ?>">
            <button type="button" onclick="location.href='../public/index.php?route=cart&product_id=<?= $product['product_id']; ?>'" name="submit" class="submit" id="addToCart">Add to cart</button>
        </a>
    </form>
</div>
</body>
